/*
 * ml-plugin: org.nrg.xnatx.plugins.datasets.rest.permissions.DataSetPermissions
        ...
        try {
            if (!_permissions.can(user, xmlPath, projectId, getAction())) {
                if (StringUtils.isNotBlank(name)) {
                    log.info("The user {} wanted to {} dataset collection \"{}\" in the project {} but doesn't have adequate permission on collections in that project.", user.getUsername(), getAction(), name, projectId);
                } else {
                    log.info("The user {} wanted to {} dataset collections in the project {} but doesn't have adequate permission on collections in that project.", user.getUsername(), getAction(), projectId);
                }
                return true;
            }
        } catch (Exception e) {
