/*
 * ml-plugin: org.nrg.xnatx.plugins.datasets.resolvers.SeriesAndResourceCriterionResolver
...
public class SeriesAndResourceCriterionResolver extends ResourceAttributeDatasetCriterionResolver {
    ...
    protected Map<String, Map<String, XnatAbstractresource>> getResources(final UserI user, final String project, final JsonNode json) {
        final String resolved = ExpressionResolver.joinClauses(getClauses(json));
        ...
        for (final String element : EXPRESSION_ATTRIBUTES.keySet()) {
            if (json.has(element)) {
                ...
                switch (node.getNodeType()) {
                    case ARRAY:
                        clauses.add(ExpressionResolver.getExpressions(EXPRESSION_ATTRIBUTES.get(element), ExpressionResolver.arrayNodeToStrings(node)));
                        break;
                    case STRING:
                    case OBJECT:
                        clauses.add(ExpressionResolver.getExpression(EXPRESSION_ATTRIBUTES.get(element), node.textValue()));
                        break;
                    default:
