/*
 * ml-plugin: org.nrg.xnatx.plugins.datasets.services.impl.xft.XftDatasetCollectionService
...
public class XftDatasetCollectionService extends AbstractXftDatasetObjectService<SetsCollection> implements DatasetCollectionService {
     ...
     */
    @Override
    public Map<String, List<HashMap<String, String>>> partitionDataset(final SetsCollection collection, final Map<String, Integer> partitions) {
        final List<HashMap<String, String>> datasetFiles = deserialize(collection.getFiles());
        ...
        final int itemCount = datasetFiles.size();
        if (total != 100 && total != itemCount) {
            throw new DatasetCollectionHandlingException("The requested partition values of " + StringUtils.join(partitions.values(), ", ") + " add up to " + accumulator + ", but must total either 100 (as percentage) or the number of items in the specified collection " + collection.getId() + " (" + itemCount + ").");
        }
        ...
        return partitions.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> datasetFiles.subList(offset.get(), offset.addAndGet(entry.getValue()))));
    }
