console.log('datasetDefinition-dev.js');

var XNAT = getObject(XNAT || {});

(function(factory){
    if (typeof define === 'function' && define.amd) {
        define(factory);
    }
    else if (typeof exports === 'object') {
        module.exports = factory();
    }
    else {
        return factory();
    }
}(function(){

    var undef;
    var plugin, collection, setsDev;

    XNAT.plugin = plugin =
        getObject(XNAT.plugin || {});

    XNAT.plugin.collection = collection =
        getObject(XNAT.plugin.collection || {});

    XNAT.plugin.collection.setsDev = setsDev =
        getObject(XNAT.plugin.collection.setsDev || {});

    var projectId = XNAT.data.context.project;
    var rootUrl   = XNAT.url.rootUrl;
    var restUrl   = XNAT.url.restUrl;
    var csrfUrl   = XNAT.url.csrfUrl;

    function probablyJSON(it){
        return /string/i.test(typeof it) && /^[{[]/.test((it).trim());
    }

    function possiblyJSON(it){
        return it && probablyJSON(it) ? JSON.parse(it) : it || '';
    }

    function errorHandler(e, title, closeAll){
        console.warn(e);
        title        = (title) ? 'Error Found: ' + title : 'Error';
        closeAll     = (closeAll === undef) ? true : closeAll;
        var errormsg = (e.statusText) ? '<p><strong>Error ' + e.status + ': ' + e.statusText + '</strong></p><p>' + e.responseText + '</p>' : e;
        XNAT.dialog.open({
            width: 450,
            title: title,
            content: errormsg,
            buttons: [
                {
                    label: 'OK',
                    isDefault: true,
                    close: true,
                    action: function(){
                        if (closeAll) {
                            xmodal.closeAll();

                        }
                    }
                }
            ]
        });
    }

    function panel(opts, items){
        return spawn('div.panel', cloneObject(opts || {}), items);
    }

    function panelItem(opts){
        opts = cloneObject(opts || {});
        return spawn('div.panel-element.panel-item', opts.panelElement, [
            ['label.element-label', opts.label],
            ['div.element-wrapper', opts.wrapper, [
                opts.contents || opts.content || '',
                opts.description ? ['div.description', opts.description] : ''
            ]],
            ['div.clear']
        ]);
    }


    // example
    function panelExample(){

        return panel({}, [
            panelItem({
                label: 'Item One',
                wrapper: {},
                contents: ['input.required.name', {
                    type: 'text',
                    name: 'name',
                    title: 'Name',
                    attr: { size: 30 }
                }],
                description: 'This is the name field.'
            }),
            panelItem({
                label: 'Email',
                wrapper: {},
                contents: ['input.required.email', {
                    type: 'text',
                    name: 'email',
                    title: 'Email',
                    attr: { size: 30 }
                }],
                description: 'This is the email field.'
            })
        ])

    }


    function panelTextInput(opts){

    }

    function definitionEditor(dfn){

        // var readonly = (opts && opts.readonly) ? opts.readonly : false;
        // var cfgLabel = opts.label;

        dfn = getObject(dfn);

        var id      = dfn.id || '';
        var criteria = dfn.criteria ? JSON.stringify(dfn.criteria) : '[]';

        var _source = spawn('textarea', criteria);
        var _editor = XNAT.app.codeEditor.init(_source, {
            language: 'json'
        });

        setsDev.criteria = _editor;

        function dfnMeta(){
            return spawn('form.definition-metadata', { style: { marginBottom: 0 } }, [
                id && ['input|type=hidden|name=id', { value: id }],
                ['input|type=hidden|name=project', { value: dfn.project || projectId }],
                ['table.xnat-table.clean.compact.borderless|style=width:100%', [
                    ['tbody', [
                        ['tr', [
                            ['td|style=width:100px', '<b>Label</b> &nbsp;'],
                            ['td', [
                                ['input.def-label|type=text|name=label|size=30', {
                                    value: dfn.label || ''
                                }]
                            ]]
                        ]],
                        ['tr', [
                            ['td.top', '<b>Description</b> &nbsp;'],
                            ['td', [
                                ['textarea.def-description|type=text|name=description', {
                                    value: dfn.description || '',
                                    style: { width: '100%' },
                                    attr: { rows: 4 }
                                }]
                            ]]
                        ]],
                        // ['tr', [
                        //     ['td.top', '<b>Resolver</b> &nbsp;'],
                        //     ['td', [
                        //         ['textarea.def-resolver|type=text|name=resolver', {
                        //             value: dfn.description || '',
                        //             style: { width: '100%' },
                        //             attr: { rows: 4 }
                        //         }]
                        //     ]]
                        // ]],
                        ['tr', [
                            ['td', '<b>Criteria</b> &nbsp;'],
                            ['td']
                        ]]
                    ]]
                ]],
                ''
            ])
        }


        // handle either pre-serialized JSON string or JSON object/array
        function processCriteria(criteria){
            return JSON.parse(criteria).map(function(item, i){
                item.payload = JSON.stringify(possiblyJSON(item.payload));
                return item;
            });
        }


        _editor.openEditor({
            title: dfn.id ? 'Edit Dataset Definition <b>' + dfn.label + '</b>' : 'New Dataset Definition',
            width: 800,
            height: 600,
            classes: 'plugin-json',
            footerContent: 'Save Dataset Definition?',
            before: dfnMeta(),
            // source: criteria,
            // beforeShow: function(dialog){
            //     dialog.body$.prepend(dfnMeta());
            // },
            // afterShow: function(dialog, obj){
            //     dialog.setHeight();
            //     // obj.aceEditor.setReadOnly(readonly);
            // },
            buttons: {
                save: {
                    label: 'Save',
                    isDefault: true,
                    close: false,
                    action: function(dialog){

                        var defData = {
                            project: projectId,
                            label: dialog.body$.find('[name="label"]').val(),
                            description: dialog.body$.find('[name="description"]').val(),
                            criteria: processCriteria(_editor.aceEditor.getValue())
                        };

                        console.log(defData);

                        // return;

                        XNAT.xhr.postJSON({
                            url: rootUrl('/xapi/sets/definitions'),
                            data: JSON.stringify(defData),
                            success: function(){
                                dialog.close();
                                XNAT.ui.banner.top(2000, 'Definition saved.', 'success');
                            }
                        })
                    }
                },
                close: {
                    label: 'Cancel',
                    close: true
                }
            }
        });

    }

    setsDev.createDefinition = function(){
        definitionEditor({});
    };

}));
